 ...
 *
 *******************************************************/
void Trackball::vCalcRotVec(float fNewX, float fNewY,
                 float fOldX, float fOldY,
                 float fDiameter,
                 float *fVecX, float *fVecY, float *fVecZ) {
 ...
 *
 *******************************************************/
void Trackball::vAxisRotMatrix(float fVecX, float fVecY, float fVecZ, QMatrix4x4 *mNewMat) {
    float fRadians, fInvLength, fNewVecX, fNewVecY, fNewVecZ;

    QMatrix4x4 mIdentity;
    ...
    if (fRadians > -0.000001 && fRadians < 0.000001) {
        *mNewMat = QMatrix4x4(mIdentity);//vCopyMatrix(mIdentity, mNewMat);
        return;
    }
    ...
    mNewMat->setRow(0, QVector4D(dCosAlpha + fNewVecX*fNewVecX*dT,
        fNewVecX*fNewVecY*dT + fNewVecZ*dSinAlpha,
        fNewVecX*fNewVecZ*dT - fNewVecY*dSinAlpha,
        0));

    mNewMat->setRow(1,
        QVector4D(fNewVecX*fNewVecY*dT - dSinAlpha*fNewVecZ,
        dCosAlpha + fNewVecY*fNewVecY*dT,
        fNewVecY*fNewVecZ*dT + dSinAlpha*fNewVecX,
        0));

    mNewMat->setRow(2,
        QVector4D(fNewVecZ*fNewVecX*dT + dSinAlpha*fNewVecY,
